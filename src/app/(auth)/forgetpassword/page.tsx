...
export default function ForgetPassword() {
  ...
  const form = useForm<LoginPayload>({resolver:zodResolver(ForgetPasswordSchema), defaultValues: { email:"" }})
...
async  function onSubmit(values:LoginPayload){
    // console.log(values)
  const res =  fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/v1/auth/forgotPasswords`,{
      method:"POST",
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            ...
            <Button className="bg-main text-white w-full" type="submit">Send Code</Button>
          </form>
        </Form>
      </div>
    </section>
